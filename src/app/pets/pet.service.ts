...
export class PetService {
  ...
  getPets(): Observable<Pet[]> {
    return this.http
      .get<Pet[]>(this.entityUrl)
      .pipe(catchError(this.handlerError('getPets', [])));
  }

  getPetById(petId: number): Observable<Pet> {
    return this.http
      .get<Pet>(this.entityUrl + '/' + petId)
      .pipe(catchError(this.handlerError('getPetById', {} as Pet)));
  }
  ...
  addPet(pet: Pet): Observable<Pet> {
    ...
    return this.http
      .post<Pet>(ownersUrl, pet)
      .pipe(catchError(this.handlerError('addPet', pet)));
  }

  updatePet(petId: string, pet: Pet): Observable<Pet> {
    return this.http
      .put<Pet>(this.entityUrl + '/' + petId, pet)
      .pipe(catchError(this.handlerError('updatePet', pet)));
  }

  deletePet(petId: string): Observable<number> {
    return this.http
      .delete<number>(this.entityUrl + '/' + petId)
      .pipe(catchError(this.handlerError('deletePet', 0)));
  }
}
